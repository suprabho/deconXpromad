 ...
 *
 * STUDIO NOTE: this is a verbatim copy of asset-showcase/lib/assets.ts. The only
 * studio-specific addition is ASSET_BASE + assetUrl(): the source `src` values are
 * root-relative ("/icons/foo.svg"), but the studio copies the public files under
 * /public/assets/, so they resolve at "/assets/icons/foo.svg". Always wrap a
 * catalogue `src` in assetUrl() before rendering.
 */

/** Public prefix where the copied asset files live in the studio app. */
export const ASSET_BASE = '/assets';

/** Resolve a catalogue `src` ("/icons/foo.svg") to its studio public URL. */
export function assetUrl(src: string): string {
  if (/^(https?:|data:)/.test(src)) return src; // already absolute
